...
MgmtLink::MgmtLink(UART_HandleTypeDef *_huart) :
		huart(_huart) {
	cobsDecodeReset();
	still_sending = false;
	rx_queue_idx = 0;
		...
		*decode++ = byte;
		...
		if (code == 0x00) {
			...
			if (decode == rx_queue[rx_queue_idx].rx_packet_buf) {
				...
				const size_t raw_len = decode - 1
						- rx_queue[rx_queue_idx].rx_packet_buf;
...
bool MgmtLink::read(EverestToMcu *msg, uint32_t timeout) {
...
bool MgmtLink::write(McuToEverest *msg) {
	...
	size_t tx_encode_len = cobsEncode(tx_packet_buf, tx_payload_len,
			encode_buf);
